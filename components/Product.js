...
import { AddToCart } from "../state_management/actions/cartActions"

function Product({ id, title, image, price, description, category, rating}) {
  ...
  return (
    <div className='relative bg-white flex flex-col p-5 m-3'>
      ...
      </div>
      {
        id === 5
          ?
          <p className="my2 text-sm my-2">{description.slice(0, 200)} {description.length > 200 ? "..." : ""}</p>
          :
          <p className="my2 text-sm my-2">{description.slice(0, 80)} {description.length > 80 ? "..." : ""}</p>
      }
